...
export default function StatisticChartItemMonthly({ flex }) {
  ...
  // Membuat array untuk menyimpan label bulan dan data total item
  const labels = [];
  const totalItemData = [];
  ...
  for (const [key, value] of Object.entries(dataChart.data.values)) {
    labels.push(key);
    totalItemData.push(value[0].total_item);
  }
